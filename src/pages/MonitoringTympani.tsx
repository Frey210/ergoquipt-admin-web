...
import type { TympaniBulkRecording, User } from '../types'
import { DEFAULT_TIMEZONE, TIMEZONE_OPTIONS, formatDateInput, toUtcIso } from '../utils/time'

const MonitoringTympani: React.FC = () => {
  const [records, setRecords] = useState<TympaniBulkRecording[]>([])
  ...
  useEffect(() => {
    const loadRecords = async () => {
      ...
      setError('')
      try {
        const response = await adminAPI.listTympaniRecordings(params)
        setRecords(response?.items || [])
        setTotal(response?.total || 0)
        setSelectedIds([])
      } catch (err: any) {
        setError(err.response?.data?.detail || 'Failed to load tympani recordings')
        setRecords([])
        setTotal(0)
      } finally {
        setLoading(false)
  ...
  const handleDownloadSingle = async (id: string, format: 'csv' | 'json') => {
    const blob = await adminAPI.downloadTympaniRecording(id, format)
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `tympani_${id}.${format}`
    link.click()
  ...
  const handleDownloadBulk = async (format: 'csv' | 'json') => {
    ...
    const blob = await adminAPI.downloadTympaniBulk(selectedIds, format)
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'tympani_bulk.zip'
    link.click()
    window.URL.revokeObjectURL(url)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Monitoring Tympani (v2)</h1>
          <p className="text-gray-600">Bulk recordings submitted by operators</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {loading ? (
          <div className="p-6 text-gray-500">Loading tympani recordings...</div>
        ) : (
...
export default MonitoringTympani
